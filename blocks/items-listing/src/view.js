/**
 * Items Listing Block - Frontend Script
 *
 * Handles infinite scroll and location filtering for item listings.
 *
 * @package Aucteeno
 */
...
/**
 * Initialize all item listing blocks on the page.
 */
function initItemsListings() {
	const listings = document.querySelectorAll(
		'.wp-block-aucteeno-items-listing[data-aucteeno-listing]'
	);
...
/**
 * Initialize a single items listing block.
 *
...
function initListing( container ) {
	...
	async function loadMore() {
		...
		try {
			...
			if ( state.auctionId ) {
				params.append( 'auction_id', state.auctionId );
			}
			...
			console.error( 'Aucteeno: Failed to load items', error );
		} finally {
			state.loading = false;
...
if ( document.readyState === 'loading' ) {
	document.addEventListener( 'DOMContentLoaded', initItemsListings );
} else {
	initItemsListings();
}
