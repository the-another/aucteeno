<?php
/**
 * Auction Datastore Class
 *
 * Datastore for auction product type with custom fields from post meta.
 *
 * @package Aucteeno
 * @since 1.0.0
 */
...
/**
 * Class Datastore_Auction
 *
 * Datastore for auction products with custom fields from post meta.
 */
class Datastore_Auction extends WC_Product_Data_Store_CPT {
	...
	public function create( &$product ): void {
		...
		// Only proceed if this is a Product_Auction instance.
		if ( ! ( $product instanceof Product_Auction ) ) {
			return;
		}

		// Save all extra_data fields for new products.
		$this->save_auction_extra_data( $product );
	}
	...
	public function update( &$product ): void {
		...
		if ( ! empty( $extra_data_changes ) ) {
			$this->save_auction_extra_data( $product, $extra_data_changes );
		}
	}
	...
	public function read( &$product ): void {
		...
		$this->read_auction_extra_data( $product );
	}

	/**
	 * Read auction extra data from database.
	 *
	 * @param Product_Auction $product Product object.
	 * @return void
	 */
	private function read_auction_extra_data( Product_Auction $product ): void {
		$product_id      = $product->get_id();
		$extra_data_keys = $product->get_extra_data_keys();
	 ...
	 * @return void
	 */
	private function read_location_data( Product_Auction $product ): void {
		$product_id = $product->get_id();
	...
	/**
	 * Save auction extra data to database.
	 *
	 * @param Product_Auction           $product Product object.
	 * @param array<string, mixed>|null $changes Changed fields. If null, saves all extra_data.
	 * @return void
	 */
	private function save_auction_extra_data( Product_Auction $product, ?array $changes = null ): void {
		$product_id = $product->get_id();
		if ( ! $product_id ) {
			return;
		}
	 ...
	 * @return void
	 */
	private function save_location_data( Product_Auction $product ): void {
		$product_id = $product->get_id();
		if ( ! $product_id ) {
			return;
		}
